...
const registerValidationEmployee = (validationData) => {
  const schema = Joi.object({
    employee_id: Joi.string().min(2),
    name: Joi.string().min(2).required(),
    contactNumber: Joi.number().min(1111111111).max(9999999999),
    email: Joi.string().min(7).required().email(),
    cci_id: Joi.string().min(4).max(30).required(),
...
const childRegistrationValidation = (validationData) => {
  const schema = Joi.object({
    ...
    cci_name: Joi.string().min(3).max(120).required(),
    cci_id: Joi.string().min(4).max(30).required(),
    address: Joi.string(),
  });
